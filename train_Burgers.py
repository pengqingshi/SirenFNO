...
@dataclass
class CFG:
    data_file: str = "Data/1D_Burgers_Sols_Nu0.001.hdf5"
    dataset_url: str = "https://darus.uni-stuttgart.de/api/access/datafile/268190"
...
def make_model(
    ...
    if model_key == "sirenfno":
        ...
        model = UFNO1d(
            ...
            out_channels=out_channels,
            n_layers=6,
            lifting_channels=2 * hidden,
            projection_channels=2 * hidden,
...
def train():
    ...
    reduced_path = preprocess_decimated_hdf5_1d_scalar(
        cfg.data_file, cfg.reduce_x, cfg.reduce_t
    )
    ...
    train_cpu = load_subset_to_ram_1d_scalar(reduced_path, train_idx)
    test_cpu = load_subset_to_ram_1d_scalar(reduced_path, test_idx)

    mean, std = compute_mean_std_from_ram(
        train_cpu, max_traj_for_stats=min(200, train_cpu.shape[0])
    )


    ds_train = RolloutRAMDataset1D(train_cpu, cfg.input_steps, cfg.rollout, mean, std)
    dl_train = DataLoader(
        ds_train,
    ...
    ds_train_eval = RolloutRAMDataset1D(
    ...
    ds_test_eval = RolloutRAMDataset1D(test_cpu, cfg.input_steps, cfg.rollout, mean, std)
    ...
    model = make_model(cfg.input_steps, cfg.modes, cfg.hidden, cfg.n_layers, cfg.model)
    ...
    for epoch in range(1, cfg.epochs + 1):
        ...
        for xb, yb in dl_train:
            xb = xb.to(cfg.device, non_blocking=True).float()
            yb = yb.to(cfg.device, non_blocking=True).float()
            opt.zero_grad(set_to_none=True)

            if cfg.use_amp:
                with torch.amp.autocast("cuda"):
                    pred_seq = rollout_step_model_1d_scalar(
                        model, xb, cfg.rollout, pushforward_detach=False
                    )

                    loss = rollout_loss_rel_l2(pred_seq, yb)
...
if __name__ == "__main__":
    p = argparse.ArgumentParser(description="FNO 1D Burgers Equation Training")
